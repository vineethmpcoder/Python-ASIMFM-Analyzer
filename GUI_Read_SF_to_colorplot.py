...
SF_file_list = file_locator.result()[1]
...
for SF_file in SF_file_list:    
    if (os.path.isfile(SF_file)==True):
        SF = MatrixImport(SF_file)
        ...
        #plt.clim(0,2)
        plt.colorbar()
        plt.savefig(SF_file.replace(SF_file.split('/')[-1],'') + "/"+ ntpath.basename(SF_file).strip( '.txt' ))

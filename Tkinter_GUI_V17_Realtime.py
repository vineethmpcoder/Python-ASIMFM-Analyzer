...
from Function_Display_ASI import DomainImage, VertexMapDI, VertexMappinDI
...
class App(object):
    def __init__(self, window, window_title):#, image_path=(foldername+'.png')):
        ...
        self.entry_color_T1 = tkinter.Checkbutton(self.frame, text="T1", variable=self.colorT1)
        self.entry_color_T1.grid(row=11,column=2)        
        self.colorT2 = tkinter.IntVar()
        self.entry_color_T2 = tkinter.Checkbutton(self.frame, text="T2", variable=self.colorT2)
        self.entry_color_T2.grid(row=12,column=2)        
        self.colorT3 = tkinter.IntVar()
        self.entry_color_T3 = tkinter.Checkbutton(self.frame, text="T3", variable=self.colorT3)
        self.entry_color_T3.grid(row=13,column=2)        
        self.colorT4 = tkinter.IntVar()
        self.entry_color_T4 = tkinter.Checkbutton(self.frame, text="T4", variable=self.colorT4)
...
#        scipy.misc.imsave( folder_path +'/Grid_' + folder_name +'.png', self.cv_img)
        ...
        template_nMx = cv2.resize(cv2.imread(template_path + '/' +'-mx.png'), (0,0), fx=image_scale, fy=image_scale)         
        template_pMx = cv2.resize(cv2.imread(template_path + '/' +'+mx.png'), (0,0), fx=image_scale, fy=image_scale)         
        template_nMy = cv2.resize(cv2.imread(template_path + '/' +'-my.png'), (0,0), fx=image_scale, fy=image_scale)         
        template_pMy = cv2.resize(cv2.imread(template_path + '/' +'+my.png'), (0,0), fx=image_scale, fy=image_scale)         


#        template_pMx = cv2.resize(cv2.imread(template_path + '/' +'+mx.png'), (0,0), fx=image_scale, fy=image_scale)         
#        (h1, w1) = template_pMx.shape[:2]
#        center = (w1/2, h1/2)
#        scale = 1.0
#        M = cv2.getRotationMatrix2D(center, 90, scale)         # Perform the counter clockwise rotation holding at the center
#        template_nMy = cv2.warpAffine(template_pMx, M, (h1, w1)) 
#        M = cv2.getRotationMatrix2D(center, 180, scale)         # Perform the counter clockwise rotation holding at the center
#        template_nMx = cv2.warpAffine(template_pMx, M, (h1, w1)) 
#        M = cv2.getRotationMatrix2D(center, 270, scale)         # Perform the counter clockwise rotation holding at the center
#        template_pMy = cv2.warpAffine(template_pMx, M, (h1, w1))



        imgX = MFM_img[int(self.Cor_matrix[self.m,self.n,1] - self.mnwidth/2):int(self.Cor_matrix[self.m,self.n,1]+ self.mnwidth/2), int(self.Cor_matrix[self.m,self.n,0])+self.X_shift:int(self.Cor_matrix[self.m,self.n,0]) + self.mnlength + self.X_shift, :]
        ...
        self.res_nMx = np.mean(cv2.matchTemplate(imgX,template_nMx,cv2.TM_CCOEFF_NORMED))#cv2.TM_CCOEFF_NORMED)    
        self.res_pMx = np.mean(cv2.matchTemplate(imgX,template_pMx,cv2.TM_CCOEFF_NORMED))#cv2.TM_CCOEFF_NORMED)    
        self.res_nMy = np.mean(cv2.matchTemplate(imgY,template_nMy,cv2.TM_CCOEFF_NORMED))#cv2.TM_CCOEFF_NORMED)    
        self.res_pMy = np.mean(cv2.matchTemplate(imgY,template_pMy,cv2.TM_CCOEFF_NORMED))#cv2.TM_CCOEFF_NORMED)    

        
        if(self.res_pMx > self.res_nMx):
            self.label_test    = tkinter.Label(self.frame,text='+Mx')
            self.label_test.grid(row=60,column=0,sticky=tkinter.S)
        else:
            self.label_test    = tkinter.Label(self.frame,text='-Mx')
            self.label_test.grid(row=60,column=0,sticky=tkinter.S)
        if(self.res_pMy > self.res_nMy):
            self.label_test    = tkinter.Label(self.frame,text='+My')
            self.label_test.grid(row=60,column=1,sticky=tkinter.S)
        else:
            self.label_test    = tkinter.Label(self.frame,text='-My')
            self.label_test.grid(row=60,column=1,sticky=tkinter.S)
...
#        scipy.misc.imsave( folder_path +'/Grid_' + folder_name +'.png', grid_image)        
    ...
    def count(self):
        ...
        template_nMx = cv2.imread(template_path + '/' +'-mx.png')#,0)
        template_pMx = cv2.imread(template_path + '/' +'+mx.png')#,0)
        template_nMy = cv2.imread(template_path + '/' +'-my.png')#,0)
        template_pMy = cv2.imread(template_path + '/' +'+my.png')#,0)
#        template_nMx = cv2.resize(cv2.imread(folder_name + '/' +'-mx.png'), (0,0), fx=image_scale, fy=image_scale)         
...
#            'cv2.TM_CCORR_NORMED', 'cv2.TM_SQDIFF', 'cv2.TM_SQDIFF_NORMED']
        self.Mx_matrix = np.zeros((self.Size_Vertex_matrix+2,self.Size_Vertex_matrix+2))
        self.My_matrix = np.zeros((self.Size_Vertex_matrix+2,self.Size_Vertex_matrix+2))
        lattice_M = self.Size_Vertex_matrix+2
        lattice_N = self.Size_Vertex_matrix+2
        mm=0
        nn=0
        for nn in range(0,self.Size_Vertex_matrix+2):    
            for mm in range(0,self.Size_Vertex_matrix+2):       
                ...
                self.res_nMx = np.mean(cv2.matchTemplate(self.imgX,template_nMx,cv2.TM_CCOEFF_NORMED))#cv2.TM_CCOEFF_NORMED)    
                self.res_pMx = np.mean(cv2.matchTemplate(self.imgX,template_pMx,cv2.TM_CCOEFF_NORMED))#cv2.TM_CCOEFF_NORMED)    
                self.res_nMy = np.mean(cv2.matchTemplate(self.imgY,template_nMy,cv2.TM_CCOEFF_NORMED))#cv2.TM_CCOEFF_NORMED)    
                self.res_pMy = np.mean(cv2.matchTemplate(self.imgY,template_pMy,cv2.TM_CCOEFF_NORMED))#cv2.TM_CCOEFF_NORMED)    
...
#        self.photo = PIL.ImageTk.PhotoImage(image = PIL.Image.fromarray(self.cv_img))
#        self.canvas.create_image(0, 0, image=self.photo, anchor=tkinter.NW)
        scipy.misc.imsave( folder_path +'/Vertex_' + folder_name +'.png', self.cv_img)
...
#########----PINNING LOCTION MAP-----------------##############################
        n0PSx = 0*n0Sx
        n0PSy = 0*n0Sx
        for j in range(0,self.Size_Vertex_matrix+2): #vertical lines
            j = self.PinStartRow + j*self.PinPeriodY
            for i in range(0,self.Size_Vertex_matrix+2): #horizontal lines
                i = self.PinStartCol + i*self.PinPeriodX
                if(i<self.Size_Vertex_matrix+1 and j<self.Size_Vertex_matrix+1):                                    
                    if(self.color_P==1):    
                        if(n0Sx[j,i] == 1):
                            n0PSx[j,i] = 1
                            n0PSy[j,i] = 0 #- self.X_step_size/2 - self.Square/2
                            self.cv_img = cv2.rectangle(self.cv_img, (int(self.X_shift  + self.Cor_matrix[j,i,0]), int(self.Cor_matrix[j,i,1]-self.Square/4)), 
                                (int(self.X_shift + self.X_step_size+ self.Cor_matrix[j,i,0]),int(self.Cor_matrix[j,i,1]+self.Square/4)), (255,255,0),-1) #cv2.FILLED                   
                        if(n0Sx[j,i] == -1):
                            n0PSx[j,i] = -1
                            n0PSy[j,i] = 0
                            self.cv_img = cv2.rectangle(self.cv_img, (int(self.X_shift  + self.Cor_matrix[j,i,0]), int(self.Cor_matrix[j,i,1]-self.Square/4)), 
                                (int(self.X_shift + self.X_step_size + self.Cor_matrix[j,i,0]),int(self.Cor_matrix[j,i,1]+self.Square/4)), (255,0,255), -1)
                        
                                  
        self.photo = PIL.ImageTk.PhotoImage(image = PIL.Image.fromarray(self.cv_img))
        self.canvas.create_image(0, 0, image=self.photo, anchor=tkinter.NW)
       
        ###########=========SAVE STATISTICS OF PINNED SITES====================
        PSx = np.zeros((2*(n0PSx.shape[0])+1,2*(n0PSx.shape[1])+1))
        print("pin=",  PSx.shape[0])
        PSx[1::2,1::2] = n0PSx
        PSx.astype(int)
        PSx = np.delete(PSx, (0), axis=0)
        PSx = np.delete(PSx, (-1), axis=1)
        
        PSy = np.zeros((2*(n0PSy.shape[0])+1,2*(n0PSy.shape[1])+1))
        PSy[1::2,1::2] = n0PSy
        PSy.astype(int)
        PSy = np.delete(PSy, (0), axis=0)
        PSy = np.delete(PSy, (-1), axis=1)
        np.savetxt(folder_path + '/'+'PSx_mfm.txt', PSx, fmt='%1i')
        np.savetxt(folder_path + '/'+'PSy_mfm.txt', PSy, fmt='%1i')
        
        X_values, X_counts = np.unique(n0PSx, return_counts=True)
        Y_values, Y_counts = np.unique(n0PSy, return_counts=True)
#        print(X_values, X_counts)

        #if(1 in X_values):#(np.count_nonzero(n0PSx) != 0):
        file.write('Count_Pin_X' + ' = '+ str(np.count_nonzero(n0PSx)) + '\n')
        file.write('%_Pin_X' + ' = '+ str(100*np.count_nonzero(n0PSx)/((self.Size_Vertex_matrix+1)*(self.Size_Vertex_matrix+1))) + '\n')
        if(-1 in X_values):
            file.write('Count_Pin_X_Rev' + ' = '+ str(X_counts[0]) + '\n')
            file.write('%_Pin_X_Rev' + ' = '+ str(100*X_counts[0]/np.count_nonzero(n0PSx)) + '\n')
            print('Count_Pin_X_Rev' + ' = '+ str(X_counts[0]) + '\n')
        else:
            file.write('Count_Pin_X_Rev' + ' = '+ '0' + '\n')
            file.write('%_Pin_X_Rev' + ' = '+ str('0') + '\n')
                    
#        else:
#            file.write('Count_Pin_X' + ' = '+ '0' + '\n')
#            file.write('Count_Pin_X_Rev' + ' = '+ '0' + '\n')
#            file.write('%_Pin_X_Rev' + ' = '+ '0' + '\n')
#            print('Count_Pin_X_Rev' + ' = '+ '0' + '\n')
            
        if(len(Y_values)>2):#(np.count_nonzero(n0PSy) != 0):
            file.write('Count_Pin_Y' + ' = '+ str(np.count_nonzero(n0PSy)) + '\n')
            file.write('Count_Pin_Y_Rev' + ' = '+ str(Y_counts[0]) + '\n')      
            file.write('%_Pin_Y_Rev' + ' = '+ str(100*Y_counts[0]/np.count_nonzero(n0PSy)) + '\n')
        else:
            file.write('Count_Pin_Y' + ' = '+ '0' + '\n')
            file.write('Count_Pin_Y_Rev' + ' = '+ '0' + '\n')
            file.write('%_Pin_Y_Rev' + ' = '+ '0' + '\n')
            
        
        ####============SAVE IMAGE WITH PINNING SITE===========================
        self.cv_img = cv2.cvtColor(cv2.imread(image_path), cv2.COLOR_BGR2RGB)
        self.cv_img = cv2.resize(self.cv_img, (0,0), fx=image_scale, fy=image_scale) 
        n0V = self.V[~(self.V==0).all(1)]
        n0V = np.transpose((np.transpose(n0V)[~np.all(np.transpose(n0V) == 0, axis=1)])) 
        ...
        for jj in range(0,self.Size_Vertex_matrix+2): #vertical lines
            j = self.PinStartRow + jj*self.PinPeriodY
            for ii in range(0,self.Size_Vertex_matrix+2): #horizontal lines
                i = self.PinStartCol + ii*self.PinPeriodX
                if(i<self.Size_Vertex_matrix+1 and j<self.Size_Vertex_matrix+1):
                    if(n0PSx[j,i] == 1): # yellow
                        self.cv_img = cv2.rectangle(self.cv_img, (int(self.X_shift  + self.Cor_matrix[j,i,0]), int(self.Cor_matrix[j,i,1]-self.Square/4)), 
                                                (int(self.X_shift + self.X_step_size + self.Cor_matrix[j,i,0]),int(self.Cor_matrix[j,i,1]+self.Square/4)), (255,255,0), -1)
                    if(n0PSx[j,i] == -1): #pink/maroon
                        self.cv_img = cv2.rectangle(self.cv_img, (int(self.X_shift + self.Cor_matrix[j,i,0]), int(self.Cor_matrix[j,i,1]-self.Square/4)), 
                                                (int(self.X_shift + self.X_step_size + self.Cor_matrix[j,i,0]),int(self.Cor_matrix[j,i,1]+self.Square/4)), (255,0,255), -1)


        scipy.misc.imsave( folder_path +'/Vertex_Pinning_' + folder_name +'.png', self.cv_img)
        
        file.close() 
###############################################################################        

        n0Vb = self.V[~(self.V==0).all(1)]
        n0Vb = np.transpose((np.transpose(n0Vb)[~np.all(np.transpose(n0Vb) == 0, axis=1)])) 
...
#        Mm, Nn = n0Vb.shape[0]+1, n0Vb.shape[1]+1 ## NxN >> (N-1)/2x(N-1)/2 square cells >> (N-3)/2 x (N-3)/2 vertex
        scipy.misc.imsave(folder_path +'/'+folder_name+'_VertexMap_DI'+'.png', VertexMapDI(n0Vb,n0V2,Sx,Sy))
#        scipy.misc.imsave(folder_path +'/'+folder_name+'_DI2_vertex'+'.png', blank_image)

        
        scipy.misc.imsave(folder_path +'/'+folder_name+'_VertexMappin_DI'+'.png', VertexMappinDI(n0Vb,n0V1,Sx,Sy,n0PSx,n0PSy))
